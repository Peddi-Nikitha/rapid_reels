...
class AdminOffersManagementScreen extends StatefulWidget {
  const AdminOffersManagementScreen({super.key});

  @override
  State<AdminOffersManagementScreen> createState() =>
      _AdminOffersManagementScreenState();
}

class _AdminOffersManagementScreenState
    extends State<AdminOffersManagementScreen> {
  ...
  Future<void> _loadOffers() async {
    ...
    if (user == null) {
      setState(() {
        _offers = [];
        _isLoading = false;
        _error = 'Please login first to access Offers Management.';
      });
      return;
    }
  ...
  Future<String?> _pickAndUploadBannerImage() async {
    try {
      ...
      final path = 'admin_banners/${user.uid}/$fileName';
